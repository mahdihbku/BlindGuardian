...
queue		= JoinableQueue(maxsize=args.CPUs)	
...
def generateLocalFiles():
	...
	results_file = open("par_camera_final_results.txt","a+")
...
def camThread():
	global persons_reps
	if args.verbose:	print("camThread: Thread started...")
	...
	while True:
		...
		if (len(reps) == 0):	# no face in frame
			if args.verbose>1:	print("camThread: No face in the frame")
			queue.task_done()
			continue
		bbs = repsAndBBs[1]
		...
		for face in reps_array:
			...
			# 		continue
			if person_id != -1:
				if args.verbose>1:	print("camThread: No new face in the frame")
				lock.release()
				continue
			...
			if args.verbose:	print("camThread: Face recognition = {} ms".format((face_reco_end_time-face_reco_start_time)*1000))
			...
			if args.verbose:	print("camThread: Computing the distance = {} ms".format((dist_comp_end-dist_comp_start)*1000))
			...
			if args.verbose:	print("camThread: Obfuscating the distance = {} ms".format((dist_obf_end-dist_obf_start)*1000))

			dist_comp_obf_end = time.time()
			if args.verbose:	print("camThread: Time(face recognition + scores computation + scores obfuscation) for {} suspects: {} ms".format(suspects_count, (dist_comp_obf_end-start_comp_time)*1000))
			if args.verbose:	print("camThread: Enc time for {} suspects: {} ms".format(suspects_count, (dist_comp_obf_end-start_enc_time)*1000))
			...
			if (data == "GET image  "):
				...
				if args.verbose:	print("camThread: Suspect detected: total rtt: {} ms".format((end_comp_time-start_comp_time)*1000))
...
# cmds:
#root@mahdi-ThinkPad-E460:/home/mahdi/workspace/OpenFaceTestEC/demos# ./client_par.py --serverIP 10.40.21.43 --CPUs 4 --verbose 2 --threshold 0.8 --load
#root@raspberrypi:/home/pi/openface/demos# ./client_par.py --serverIP 10.40.21.38 --CPUs 4 --Gportion 1 --verbose 2 --serverPort 12345 --threshold 0.8 --load
